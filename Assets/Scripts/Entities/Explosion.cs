...
public class Explosion : MonoBehaviour
{
    ...
    public static void Explode(Vector2 position, float radius, AttackInfo info, float damageLerp = 0.5f)
    {
        Effect.Play("Explosion1", EffectInfo
            .PosRotScale(position + Random.insideUnitCircle * radius, 0, radius * 1.5f)
            .SetLayer("Overlay"));
        ...
        for (int i = 0; i < 4; i++)
        {
            Utility.GetMono().Delay(Random.Range(0.1f, 0.3f),
                ...
                .PosRotScale(position + Random.insideUnitCircle * radius, Random.Range(-180, 180), Random.Range(0.2f, 1f))
                .SetLayer("Overlay")
                ));
        }
        for (int i = 0; i < 1; i++)
        ...
        CamController.main.Shake(0.07f);
        ...
        foreach (Collider2D hit in hits)
        {
            if (hit != null && hit.TryGetComponent(out Hp hp))
            {
                ...
                info.knockBack = Bullet.DEFAULT_KNOCKBACK;
                hp.Damage(info);
            }
        }
    }
